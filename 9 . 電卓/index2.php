...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['sum'])) {
        ...
        $patarnNumber = "/[\d]/";
        $separateOperator = preg_split($patarnNumber, $display);

        if ($separateOperator[1] == '-') {
            $total = (int)$separateNumber[0] - (int)$separateNumber[1];
            if ($separateOperator[2] == '+') {
                $total += (int)$separateNumber[2];
            } elseif ($separateOperator[2] == '*') {
                $total *= (int)$separateNumber[2];
            } elseif ($separateOperator[2] == '/') {
                $total /= (int)$separateNumber[2];
            } elseif ($separateOperator[2] == '-') {
